...
export class ResumoFormPageComponent implements OnInit,
  ...
  private async loadClientes() {
    ...
    this.subscription.add(
      this.clienteService.getClientesQTD().subscribe((response) => {
        this.clientes = response;
        this.loadingService.off();
        this.contador = parseInt(response.toString());
  ...
  private async loadPassagens() {
    this.loadingService.on();
    this.subscription.add(
      this.passagemService.getPassagensQTD().subscribe((response) => {
        this.passagens = response;
        this.loadingService.off();
        this.contadorPassagem = parseInt(response.toString());
        console.log(response);
      })
    );
  }
  ...
  private async loadPacotes() {
    this.loadingService.on();
    this.subscription.add(
      this.pacoteService.getPacotesQTD().subscribe((response) => {
        this.pacotes = response;
        this.loadingService.off();
        this.contadorPacote = parseInt(response.toString());
        console.log(response);
      })
    );
  }
